...
/**
 * Centralized styling constants for the Chat UI.
...
public class ChatStyles {
    ...
    private void initializeFonts() {
        // Code font - monospace
        codeFont = new Font(display, new FontData[] {
                new FontData("Consolas", 10, SWT.NORMAL),
                new FontData("Courier New", 10, SWT.NORMAL) // Fallback
        });

        // Header font - bold
        FontData[] systemFont = display.getSystemFont().getFontData();
        headerFont = new Font(display, systemFont[0].getName(),
                systemFont[0].getHeight(), SWT.BOLD);

        // Normal font
        normalFont = new Font(display, systemFont[0].getName(),
                systemFont[0].getHeight(), SWT.NORMAL);
    }
